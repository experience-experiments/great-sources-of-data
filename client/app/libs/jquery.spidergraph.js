(function($){ 
    ...
    var methods = {
        ...
        // RMA extended methods
        applyActiveData: function(layerName) {
            var data = $(this).data('spidergraph');

            if  (data.activedata.name) {
                data.fixeddata.push(data.activedata);
            }

            // loop over fixed data and set the current active and remove it
            for ( var x=0; x<data.fixeddata.length; x++ )
            {
                console.log('Fixed Data contains ' + data.fixeddata[x].name);
                if (data.fixeddata[x].name == layerName){
                    console.log('setting active data to ' + data.fixeddata[x].name );
                    data.activedata = data.fixeddata[x];
                    //remove the active data from fixeddata
                    data.fixeddata.splice(x,1);
                }
            }
